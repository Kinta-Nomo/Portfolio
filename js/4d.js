...
function draw(){
    ...
    for (const hyperobject of data){
        beginShape();
        for (const point of hyperobject){
            point.rotate(angles[0], angles[1], angles[2], angles[3], angles[4], angles[5])
            point.draw()
        }
        endShape(CLOSE);
    }
    ...
    if(keyIsDown(16)) playerPos[1] += 5;
    if(keyIsDown(32)) playerPos[1] -= 5;
    
    if(keyIsDown(87)) playerPos[2] -= 5;
    if(keyIsDown(83)) playerPos[2] += 5;
    
    if(keyIsDown(68)) playerPos[0] += 5;
    if(keyIsDown(65)) playerPos[0] -= 5;

    if(keyIsDown(81)) playerPos[3] += 5;
    if(keyIsDown(69)) playerPos[3] -= 5;
}